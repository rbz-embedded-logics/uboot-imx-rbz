...
__weak unsigned int get_hw_version(void)
{
  ...
  unsigned int hw_version = 2;
  int offset;

  offset = fdt_path_offset(gd->fdt_blob, "/chosen");
  printf("offset hw_version: %d\n", offset);
  if (offset >= 0) {
    hwversion = fdt_getprop(gd->fdt_blob, offset, "hw-version", NULL);
    printf("hw: %s\n", hwversion);

    if (strcmp(hwversion, "3"))
      hw_version = 3;
    else
      hw_version = 2;
  }
...
static int setup_fec(void)
{
    ...
    setbits_le32(&gpr->gpr[1], BIT(19) | BIT(21));clrsetbits_le32(&gpr->gpr[1],
        IOMUXC_GPR_GPR1_GPR_ENET_QOS_INTF_SEL_MASK, BIT(16));
